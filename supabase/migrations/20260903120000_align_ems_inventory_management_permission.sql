...
begin
  ...
  if p_transaction_type in ('Restock', 'Return')
    and not public.member_has_app_permission(v_department_id, 'inventory_management') then
...
create or replace function public.apply_ems_supply_correction(
...
declare
  v_auth_user_id uuid := auth.uid();
  v_department_id uuid;
  v_member_in_department_id uuid;
  v_manager_member_id uuid;
  v_quantity_before numeric(12,2);
  v_quantity_after numeric(12,2);
...
begin
  ...
  if p_supply_item_id is null then
    raise exception 'Supply item id is required.';
  end if;
  ...
  select s.department_id, s.quantity_on_hand, s.unit_of_measure
  into v_department_id, v_quantity_before, v_unit_of_measure
  from public.ems_supply_items s
  where s.id = p_supply_item_id
  for update;
  ...
  select audm.member_id
  into v_member_in_department_id
  from public.auth_user_department_memberships audm
  join public.members m
    on m.id = audm.member_id
  where audm.auth_user_id = v_auth_user_id
  ...
  if v_member_in_department_id is null then
    raise exception 'Supply item does not belong to your department.';
  end if;

  if not public.member_has_app_permission(v_department_id, 'inventory_management') then
    raise exception 'Forbidden.';
  end if;

  v_manager_member_id := v_member_in_department_id;
  v_quantity_after := p_new_quantity::numeric(12,2);
  v_quantity_delta := v_quantity_after - v_quantity_before;

  update public.ems_supply_items
  set quantity_on_hand = v_quantity_after,
      updated_by_member_id = v_manager_member_id
  ...
  insert into public.ems_supply_adjustments (
    department_id,
    supply_item_id,
...
revoke all on function public.apply_ems_supply_correction(uuid, numeric, text, text) from public;
revoke all on function public.apply_ems_supply_correction(uuid, numeric, text, text) from anon;
grant execute on function public.apply_ems_supply_correction(uuid, numeric, text, text) to authenticated;
