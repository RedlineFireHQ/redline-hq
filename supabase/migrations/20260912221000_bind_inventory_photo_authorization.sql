-- Apply the Phase 1 inventory-parent lookup to the already-created storage helper.
...
begin
  ...
  -- Current inventory uploaders do not bind a parent ID until after upload.
  return v_operation = 'select';
end;
$$;

comment on function public.can_manage_storage_object(text, text, text) is
  'Authorizes department-documents storage operations by resolving document revisions to their source or requiring a recognized creation route. Existing document objects use source permissions; existing inventory photos resolve to their parent row and category permission; new Pre-Plan objects use the explicit active-member creation exception; unbound inventory writes are denied because current paths lack parent identity; path prefixes alone are never sufficient.';
