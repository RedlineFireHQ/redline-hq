...
export async function GET(_: Request, context: RouteContext) {
	try {
		...
		if (itemError) {
			return jsonResponse({ ok: false, error: itemError.message || "Unable to load equipment deficiencies." }, 400);
		}
		...
		const { data, error } = await supabase
			...
			.eq("misc_fire_equipment_id", id)
			.order("reported_at", { ascending: false });

		if (error) {
			return jsonResponse({ ok: false, error: error.message || "Unable to load equipment deficiencies." }, 400);
		}
		...
		const message = error instanceof Error ? error.message : "Unable to load equipment deficiencies.";
