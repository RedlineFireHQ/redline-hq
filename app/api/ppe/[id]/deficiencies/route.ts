...
export async function GET(_: Request, context: RouteContext) {
  try {
    ...
    if (error) {
      return jsonResponse({ ok: false, error: error.message || "Unable to load PPE deficiencies." }, 400);
    }
    ...
    const message = error instanceof Error ? error.message : "Unable to load PPE deficiencies.";
