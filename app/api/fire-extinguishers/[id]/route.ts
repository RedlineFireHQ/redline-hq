...
type UpdateFireExtinguisherPayload = {
...
export async function GET(_: Request, context: RouteContext) {
	try {
		...
		if (error) {
			return jsonResponse({ ok: false, error: error.message || "Unable to load fire extinguisher." }, 400);
		}
		...
		const message = error instanceof Error ? error.message : "Unable to load fire extinguisher.";
...
export async function PATCH(request: Request, context: RouteContext) {
	try {
		const { id } = await context.params;
		const payload = (await request.json().catch(() => ({}))) as UpdateFireExtinguisherPayload;
		const supabase = await createSupabaseServerClient();
		const currentMember = await getCurrentMember(supabase);
		...
		if (existingError) {
			return jsonResponse({ ok: false, error: existingError.message || "Unable to update fire extinguisher." }, 400);
		}
		...
		if (error || !data) {
			...
			return jsonResponse({ ok: false, error: error?.message || "Unable to update fire extinguisher." }, 400);
		}
		...
		const message = error instanceof Error ? error.message : "Unable to update fire extinguisher.";
		return jsonResponse({ ok: false, error: message }, 400);
	}
}
...
export async function DELETE(_: Request, context: RouteContext) {
	try {
		...
		if (existingError) {
			return jsonResponse({ ok: false, error: existingError.message || "Unable to delete fire extinguisher." }, 400);
		}
		...
		if (deleteError) {
			return jsonResponse({ ok: false, error: deleteError.message || "Unable to delete fire extinguisher." }, 400);
		}
		...
		const message = error instanceof Error ? error.message : "Unable to delete fire extinguisher.";
		return jsonResponse({ ok: false, error: message }, 400);
	}
}
