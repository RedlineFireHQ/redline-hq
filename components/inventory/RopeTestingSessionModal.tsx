...
export type RopeTestingStatus = "untested" | "passed" | "failed";

export type RopeTestingSessionValues = {
  testingDate: string;
  tester: string;
  ropeStatuses: Record<string, RopeTestingStatus>;
};

export type RopeTestingItem = {
  id: string;
...
type RopeTestingSessionModalProps = {
  isOpen: boolean;
  ropes: RopeTestingItem[];
  defaultTester: string;
  departmentName?: string;
  onClose: () => void;
  onSave: (values: RopeTestingSessionValues) => Promise<boolean>;
  onCreateDeficiencies: (
    failedRopes: RopeTestingItem[],
    values: RopeTestingSessionValues,
  ) => Promise<boolean>;
  resumeValues?: RopeTestingSessionValues | null;
};
...
export function getFailedRowsMissingDeficiency(
  rows: RopeTestingItem[],
  ropeStatuses: Record<string, RopeTestingStatus>,
) {
  return rows.filter((row) => {
    const status = ropeStatuses[row.id] ?? "untested";
    if (status !== "failed") {
      return false;
    }

    return row.hasActiveDeficiency !== true;
  });
}

export default function RopeTestingSessionModal({
  isOpen,
  ropes,
  defaultTester,
  ...
  onCreateDeficiencies,
  resumeValues = null,
}: RopeTestingSessionModalProps) {
  const [testingDate, setTestingDate] = useState(todayIsoDate());
  const [tester, setTester] = useState("");
  const [ropeStatuses, setRopeStatuses] = useState<Record<string, RopeTestingStatus>>({});
  ...
  const [quickSelectedRopeId, setQuickSelectedRopeId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (resumeValues) {
      ...
      setRopeStatuses(
        Object.fromEntries(
          ropes.map((rope) => [rope.id, resumeValues.ropeStatuses[rope.id] ?? "untested"]),
        ),
      );
      ...
      setRopeStatuses(Object.fromEntries(ropes.map((rope) => [rope.id, "untested"])));
    ...
    setMode("session");
    setQuickInput("");
    setQuickSelectedRopeId(null);
  }, [isOpen, defaultTester, resumeValues]);

  const selectedCount = useMemo(
    () => Object.values(ropeStatuses).filter((status) => status !== "untested").length,
    [ropeStatuses],
  );

  const quickMatches = useMemo(() => {
    const normalized = normalizeText(quickInput);
    if (!normalized) {
      return [] as RopeTestingItem[];
    }
  ...
  useEffect(() => {
    ...
    const exactMatch = ropes.find((rope) => {
      const possibleMatches = [rope.id, rope.ropeIdentifier, rope.ropeName].map(normalizeText);
      return possibleMatches.some((field) => field === normalizeText(trimmedInput));
    });

    if (exactMatch) {
      setQuickSelectedRopeId(exactMatch.id);
      return;
    }

    if (quickMatches.length === 1) {
      setQuickSelectedRopeId(quickMatches[0].id);
      return;
    }
    ...
    setQuickSelectedRopeId(null);
  }, [mode, quickInput, quickMatches, ropes]);

  const summaryRows = useMemo(
    () =>
      ropes.map((rope) => ({
        ...rope,
        status: ropeStatuses[rope.id] ?? "untested",
      })),
    [ropeStatuses, ropes],
  );
  ...
  const quickPassedCount = useMemo(
    () => Object.values(ropeStatuses).filter((status) => status === "passed").length,
    [ropeStatuses],
  );

  const quickFailedCount = useMemo(
    () => Object.values(ropeStatuses).filter((status) => status === "failed").length,
    [ropeStatuses],
  );

  const failedRowsMissingDeficiency = useMemo(
    () => getFailedRowsMissingDeficiency(summaryRows, ropeStatuses),
    [summaryRows, ropeStatuses],
  );

  const hasBlockingFailedRopes = failedRowsMissingDeficiency.length > 0;
  const createDeficienciesDisabled =
    failedRowsMissingDeficiency.length === 0 || isFinishing || isCreatingDeficiencies;
  const finishDisabled = selectedCount === 0 || !testingDate || isFinishing || hasBlockingFailedRopes;
  const quickFinishDisabled = selectedCount === 0 || !testingDate || isFinishing || hasBlockingFailedRopes;

  if (!isOpen) {
    return null;
  }

  const toggleStatus = (ropeId: string, target: "passed" | "failed") => {
    setRopeStatuses((current) => {
      const existing = current[ropeId] ?? "untested";
  ...
  const quickSelectedRope = ropes.find((rope) => rope.id === quickSelectedRopeId) ?? null;
  const quickRemainingCount = Math.max(0, ropes.length - quickPassedCount - quickFailedCount);

  const selectQuickRope = (rope: RopeTestingItem) => {
    setQuickSelectedRopeId(rope.id);
    setQuickInput(rope.ropeIdentifier);
  };
  ...
  const createPrintHtml = () => {
    ...
    const detailRows = summaryRows
      .map((row) => {
        ...
        return `<tr><td>${row.ropeIdentifier}</td><td>${row.ropeName}</td><td>${row.ropeType}</td><td>${row.length}</td><td>${resultLabel}</td></tr>`;
      })
      .join("");
...
<head>
  <meta charset="utf-8" />
  <title>Rope Inspection Summary</title>
  <style>
...
<body>
  <h1>REDLINE HQ</h1>
  <h2 style="margin-top:8px;">Rope Inspection Summary</h2>
  <div class="section muted">
    <div>${departmentName}</div>
    <div>Inspection Date: ${reportDate}</div>
    <div>Inspector: ${tester || "-"}</div>
  </div>

  <div class="section">
    <h2 style="font-size:16px;">Summary</h2>
    <div class="summary-grid">
      <div class="summary-item"><div class="muted">Total Ropes</div><div>${summaryRows.length}</div></div>
  ...
  const saveThenCreateDeficiencies = async () => {
    ...
    const saved = await onCreateDeficiencies(
      failedRowsMissingDeficiency.map((row) => ({
        id: row.id,
        ropeIdentifier: row.ropeIdentifier,
        ropeName: row.ropeName,
        ropeType: row.ropeType,
        length: row.length,
  ...
  const handleQuickMark = (status: "passed" | "failed") => {
    if (!quickSelectedRopeId) {
      return;
    }

    setRopeStatuses((current) => ({
      ...current,
      [quickSelectedRopeId]: status,
    }));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/70 px-4 py-6">
      <div className="max-h-[calc(100vh-3rem)] w-full max-w-4xl overflow-y-auto rounded-2xl border border-neutral-800 bg-[#2E2E2E] p-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-black text-white">Rope Inspection Session</h3>
        </div>
        ...
        {sessionStep === "summary" ? (
          <div className="mt-4 rounded-lg border border-white/10 bg-[#1b1b1b] p-4">
            <h4 className="text-lg font-black text-white">Rope Inspection Summary</h4>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Inspection Date</p>
                <p className="mt-1 text-sm text-neutral-200">{formatSummaryDate(testingDate)}</p>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Inspector</p>
                ...
                <div className="mt-2 space-y-1">
                  {passedRows.map((row) => (
                    <div key={row.id} className="rounded-md border border-green-700/30 bg-green-900/10 px-3 py-2 text-sm text-green-100">
                      PASS {row.ropeIdentifier} {"  "}{row.ropeName} {"  "}{row.length}
                    </div>
                  ))}
                  {passedRows.length === 0 ? <p className="text-xs text-neutral-500">No passed ropes.</p> : null}
                </div>
              </div>
                ...
                <div className="mt-2 space-y-1">
                  {failedRows.map((row) => (
                    <div key={row.id} className="rounded-md border border-red-700/30 bg-red-900/10 px-3 py-2 text-sm text-red-100">
                      FAIL {row.ropeIdentifier} {"  "}{row.ropeName} {"  "}{row.length}
                    </div>
                  ))}
                  {failedRows.length === 0 ? <p className="text-xs text-neutral-500">No failed ropes.</p> : null}
                </div>
              </div>
                ...
                <div className="mt-2 space-y-1">
                  {untestedRows.map((row) => (
                    <div key={row.id} className="rounded-md border border-amber-700/30 bg-amber-900/10 px-3 py-2 text-sm text-amber-100">
                      OPEN {row.ropeIdentifier} {"  "}{row.ropeName} {"  "}{row.length}
                    </div>
                  ))}
                  {untestedRows.length === 0 ? <p className="text-xs text-neutral-500">No untested ropes.</p> : null}
                </div>
              </div>
            </div>

            <div className="mt-6 flex flex-wrap items-center justify-end gap-2">
              {hasBlockingFailedRopes ? (
                <p className="mr-auto text-xs font-semibold uppercase tracking-[0.12em] text-amber-300">
                  Failed ropes require deficiencies before finishing.
                </p>
              ) : null}
              <button
                type="button"
          ...
          <>
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              ...
              <label className="grid gap-1.5 md:col-span-2">
                <span className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Inspector</span>
                <input
                  type="text"
                  value={tester}
            ...
            <div className="mt-4 rounded-lg border border-white/10 bg-[#1b1b1b] p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">Ropes</p>
                <p className="text-xs text-neutral-500">Tap PASS or FAIL. Tap again to clear back to Untested.</p>
              </div>

              <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
                {ropes.map((rope) => {
                  const status = ropeStatuses[rope.id] ?? "untested";
                  ...
                  return (
                    ...
                    >
                      <span className="font-semibold text-white">{rope.ropeIdentifier}</span>
                      <span className="text-neutral-300">{rope.ropeType}</span>
                      <span className="text-neutral-400">{rope.length}</span>
                      <div className="ml-auto flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => toggleStatus(rope.id, "passed")}
                        ...
                        <button
                          type="button"
                          onClick={() => toggleStatus(rope.id, "failed")}
                          className={`inline-flex items-center justify-center rounded-md border px-2.5 py-1.5 text-xs font-semibold transition ${
                            isFailed
              ...
              <p className="mt-3 text-sm font-semibold text-neutral-200">Selected: {selectedCount} ropes</p>
            </div>
          ...
          <div className="mt-4 rounded-lg border border-white/10 bg-[#1b1b1b] p-4">
            <h4 className="text-lg font-black text-white">Rope Quick Test</h4>
            <div className="mt-4">
              ...
              {quickInput.trim().length > 0 && quickMatches.length > 0 && (
                <div className="mt-2 max-h-40 overflow-y-auto rounded-lg border border-white/10 bg-[#121212]">
                  {quickMatches.map((rope) => (
                    <button
                      key={rope.id}
                      type="button"
                      onClick={() => selectQuickRope(rope)}
                    ...
                    >
                      {rope.ropeIdentifier} - {rope.ropeName}
                    </button>
                  ))}
                </div>
              )}
              ...
              {quickSelectedRope ? (
                <p className="mt-2 text-xs text-neutral-400">
                  Selected: {quickSelectedRope.ropeIdentifier} - {quickSelectedRope.ropeName} - {quickSelectedRope.ropeType} - {quickSelectedRope.length}
                </p>
              ) : null}
            </div>
