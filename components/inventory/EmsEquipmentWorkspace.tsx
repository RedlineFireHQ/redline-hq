...
type EquipmentDetailResponse = {
  item: EmsEquipmentRow;
  photoUrl: string | null;
};

type EquipmentDeficiencyRow = {
  id: string;
...
type EquipmentDeficienciesResponse = {
  rows: EquipmentDeficiencyRow[];
};
...
function statusBadgeClasses(status: EquipmentStatus) {
  if (status === "Active") {
...
function toFormValues(row: EmsEquipmentRow): EmsEquipmentFormValues {
  return {
    equipmentName: row.equipment_name,
    manufacturer: row.manufacturer ?? "",
    model: row.model ?? "",
    serialNumber: row.serial_number ?? "",
    assetId: row.equipment_number ?? "",
...
export default function EmsEquipmentWorkspace({
  ...
  const [rows, setRows] = useState<EmsEquipmentRow[]>(initialRows);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<StatusFilter>("All");
  const [toastMessage, setToastMessage] = useState<string | null>(initialError);

  const [selectedItemId, setSelectedItemId] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<EmsEquipmentRow | null>(null);
  const [selectedPhotoUrl, setSelectedPhotoUrl] = useState<string | null>(null);
  const [selectedDeficiencies, setSelectedDeficiencies] = useState<EquipmentDeficiencyRow[]>([]);
  ...
  const launchDeficiencyReport = () => {
    ...
    const params = new URLSearchParams({
      inventoryCategory: "ems-equipment",
      inventoryItemId: selectedItem.id,
      inventoryItemLabel: selectedItem.equipment_name,
      returnTo: "/inventory/ems-equipment",
    });
  ...
  const loadDetail = async (itemId: string) => {
    ...
    try {
      const [detailResponse, deficienciesResponse] = await Promise.all([
        fetch(`/api/ems/equipment/${itemId}`, { method: "GET" }),
        fetch(`/api/ems/equipment/${itemId}/deficiencies`, { method: "GET" }),
      ]);

      const payload = (await detailResponse.json().catch(() => ({}))) as
        | ({ error?: unknown } & Partial<EquipmentDetailResponse>)
        | null;

      if (!detailResponse.ok || !payload || !payload.item) {
        setToastMessage(normalizeApiError(payload, "Unable to load EMS equipment detail."));
        return;
      }
      ...
      const deficienciesPayload = (await deficienciesResponse.json().catch(() => ({}))) as
        | ({ error?: unknown } & Partial<EquipmentDeficienciesResponse>)
        | null;
      ...
      setToastMessage("Unable to load EMS equipment detail.");
    } finally {
  ...
  const saveEquipment = async (values: EmsEquipmentFormValues) => {
    ...
    const endpoint = isAdd ? "/api/ems/equipment" : `/api/ems/equipment/${selectedItem?.id ?? ""}`;
    ...
    try {
      ...
      setRows((current) => {
        if (isAdd) {
          return [nextItem, ...current].sort((left, right) => compareEquipmentNames(left.equipment_name, right.equipment_name));
        }
  ...
  const deleteEquipment = async () => {
    ...
    const confirmed = window.confirm("Delete this EMS equipment item?");
    if (!confirmed) {
      return;
    }
    ...
    try {
      const response = await fetch(`/api/ems/equipment/${selectedItem.id}`, {
        method: "DELETE",
      });
      ...
      if (!response.ok) {
        setToastMessage(normalizeApiError(payload, "Unable to delete EMS equipment."));
        return;
      }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <h1 className="mt-2 text-[2.25rem] font-[700] leading-none tracking-[-0.06em] text-white">EMS Equipment</h1>
        <p className="mt-3 max-w-2xl text-lg text-neutral-400">
          Durable EMS equipment asset records with lifecycle and location visibility.
        </p>
        <p className="mt-1 text-sm text-neutral-500">
          {departmentName ? `${departmentName} Equipment Management` : "Equipment Management"}
        </p>
      </div>
      ...
      <section className="rounded-2xl border border-neutral-800 bg-[#2E2E2E] p-5">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0 flex-1">
            <label htmlFor="ems-equipment-search" className="sr-only">
              Search EMS equipment
            </label>
            <input
              id="ems-equipment-search"
              type="text"
              value={searchTerm}
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-start">
        <div className="rounded-2xl border border-neutral-800 bg-[#2E2E2E] p-0 overflow-hidden">
          <div className="max-h-[56vh] overflow-auto">
            ...
            {filteredRows.length === 0 ? (
              <div className="px-5 py-8 text-center text-sm text-neutral-500">
                No EMS equipment found for the current filters.
              </div>
            ) : null}
          </div>
        </div>
          ...
          <p className="text-xs uppercase tracking-[0.16em] text-neutral-500">Equipment Detail</p>

          {!selectedItemId ? (
            <p className="mt-4 text-sm text-neutral-400">
              Select an EMS equipment item to review details.
            </p>
          ) : isDetailLoading ? (
            <p className="mt-4 text-sm text-neutral-400">Loading detail...</p>
          ) : !selectedItem ? (
            <p className="mt-4 text-sm text-neutral-400">No equipment detail available.</p>
          ) : (
            <div className="mt-4 space-y-4">
              {selectedPhotoUrl ? (
                <img
                  src={selectedPhotoUrl}
                  alt={`${selectedItem.equipment_name} photo`}
              ...
              <div>
                <h3 className="text-xl font-black text-white">{selectedItem.equipment_name}</h3>
                <p className="mt-1 text-sm text-neutral-400">
                  {[selectedItem.manufacturer, selectedItem.model].filter(Boolean).join(" • ") || "No manufacturer/model"}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="mt-1 text-neutral-200">{selectedItem.equipment_number || "-"}</p>
                </div>
                <div>
                  <p className="text-[11px] uppercase tracking-[0.12em] text-neutral-500">Placed In Service</p>
      ...
      <EmsEquipmentFormModal
        ...
        onDelete={canManageEquipment ? () => void deleteEquipment() : undefined}
      />
    </div>
  );
}
